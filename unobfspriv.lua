...
section1:addToggle("Noclip (reset to force turn off)", nil, function(nc)
local noclip = (nc) char = game.Players.LocalPlayer.Character while true do if (nc) == true then for _,v in pairs(char:children()) do pcall(function() if v.className == "Part" then v.CanCollide = false elseif v.ClassName == "Model" then v.Head.CanCollide = false end end) end end game:service("RunService").Stepped:wait() end
end)
